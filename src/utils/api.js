...
export const fetchAnatomyTerms = async () => {
  ...
  return terms.filter(t => t && typeof t === 'object').map(t => {
    // İç içe geçmiş bozuk bir yapı varsa düzelt
    if (t.isim && typeof t.isim === 'object') {
      return { ...t, isim: t.isim.isim || "" };
    }
    return t;
  });
};
